	...
	WebSocketConnect: function(instanceId) {
		...
		instance.ws.onmessage = function(ev) {
			...
			if (!webSocketState.onMessage) {
				console.error("[JSLIB WebSocket] onMessage callback is not set!");
				return;
			}

			var msg;
			if (ev.data instanceof ArrayBuffer) {
				msg = new TextDecoder("utf-8").decode(new Uint8Array(ev.data));
			} else {
				msg = ev.data; // Уже строка
			}
			...
			try {
				Module['dynCall_viii'](webSocketState.onMessage, instanceId, msgBuffer, msgBytes);
			} finally {
				_free(msgBuffer);
			}
		};

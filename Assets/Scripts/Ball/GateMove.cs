...
public class GateMove : MonoBehaviour
{
    ...
    private void CreateControls()
    {
        ...
        th.OnSwipe += (swipeCoords) =>
        {
            if (gameController.playerRole == PlayerTypes.kicker)
            {
                var simpleCoords = SimplifySwipeLine(swipeCoords);
                if (simpleCoords != null)
                {
                    var message = new KickStartMessage(simpleCoords,gameController.selectedStrikeType);
                    webGameManager.SendMessage(message);
                }
            }
        };
             
        webGameManager.OnKickStartReceived += (message) => 
        {      
            if (gameController.playerRole == PlayerTypes.goalkeeper)
            {
                gameController.selectedStrikeType = (StrikeType) Enum.Parse(typeof(StrikeType), message.strikeType, true); 

                OnSwipe(message.swipeCoords); 
                Debug.Log("Swipe recieved " + message.swipeCoords);
            }
        };
    ...
    private void OnSwipe(Vector3[] simpleCoords)
    {
        if (!isMoving)
        {
            isMoving = true;
    ...
    private Vector3[] SimplifySwipeLine(Vector2[] swipeLine)
    {
        ...
        if (simplify.Length >= 5)
        {
            ...
            if (simpleCoords[4].y > simpleCoords[0].y)
            {
                return simpleCoords;
            }
        }
    ...
    private IEnumerator MoveToGate(Vector3[] moveCoords)
    {
        ...
        if (moveCoords[4].y > moveCoords[0].y) // Если свайп был вверх
        {
            ...
            webGameManager.SendMessage(new KickLateMessage());
